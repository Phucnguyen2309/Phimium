    ...
    >
      ...
      <div className="flex min-h-[256px] flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="rounded bg-indigo-100 px-2 py-1 text-[11px] font-medium text-indigo-700">
            {activity.category}
          </span>
        </div>
...
/>
          ...
          <span>Led by {activity.hostName}</span>
        </div>
        ...
        {isCompleted ? (
          <div className="mt-auto">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link
                to={`/review/${activity.registrationId || activity.id}`}
                className="flex items-center justify-center rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500 shadow-sm hover:bg-slate-50 transition"
              >
                Write review
              </Link>
          ...
          <div className="mt-auto">
            ...
            {isInProgress ? (
              ...
              <div className="mt-5 grid grid-cols-2 gap-2">
                ...
                <Link
                  to="/chat"
                  className="flex items-center justify-center gap-2 rounded-md bg-blue-600 px-4 py-3 text-xs font-semibold text-white hover:bg-blue-700 transition"
                >
                  Message
                </Link>
              </div>
