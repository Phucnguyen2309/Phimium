...
const mapActivity = (activity, index) => {
  ...
  return {
    ...
    minimumParticipants:
      activity?.minimumParticipants ?? fallback.minimumParticipants,
    maximumParticipants:
      activity?.maximumParticipants ?? fallback.maximumParticipants,
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-8 lg:p-10">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600 sm:text-base">
            Mỗi hoạt động hiển thị tên, hình ảnh, thời gian và số slot còn
            trống. Chọn card bất kỳ để xem chi tiết.
          </p>
        </div>
      </section>
        ...
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Hiển thị từ API khi đã login, ngược lại dùng dữ liệu mẫu.
            </p>
          </div>
          <Link
            to={ROUTES.login}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">
                      {activity.participationFee > 0
                        ? `${activity.participationFee.toLocaleString('vi-VN')} VND`
                        : 'Free'}
                    </span>
                  </div>
