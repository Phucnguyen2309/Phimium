...
function BackButton() {
  ...
  return (
    <button
      type="button"
      onClick={() => navigate(-1)}
      className="mb-5 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
    >
      <svg
        className="h-4 w-4"
        viewBox="0 0 24 24"
...
function Avatar({ name, avatarUrl, large = false, outlined = false }) {
  ...
  return (
    <div
      className={`relative flex ${sizeClass} items-center justify-center rounded-full bg-blue-100 font-black text-blue-700 ${
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <div className="relative">
        ...
        {isCurrentUser && (
          <span className="absolute bottom-0 right-0 h-4 w-4 rounded-full border-2 border-white bg-blue-500" />
        )}
      </div>
